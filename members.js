    ...
    container.innerHTML = `
      <div class="page-header">
        <div class="page-header-left">
          <div class="page-title">உறுப்பினர் சேர்க்கை & பதிவேடு (Member Registry)</div>
          <div class="page-subtitle">Don Bosco Youth Centre (Basin Bridge, Chennai - 600 012) &bull; தொன்போஸ்கோ இளைஞர் மன்றம்</div>
        </div>
        <div class="page-actions">
          ${canManage ? '<button class="btn btn-primary btn-sm" onclick="Members.openAddModal()">➕ சேர்க்கை (New Admission)</button>' : ''}
          ${canManage ? '<button class="btn btn-outline btn-sm" onclick="Members.openDocumentScannerModal()">📑 படிவம் ஸ்கேன் (Scan Form)</button>' : ''}
          <button class="btn btn-outline btn-sm" onclick="VoiceKeyboard.toggleKeyboard()">⌨️ விசைப்பலகை (Keys)</button>
          <button class="btn btn-ghost btn-sm" onclick="Members.exportCSV()">📥 Export CSV</button>
        </div>
      </div>
      ...
      <div class="card" style="margin-bottom:var(--s-4);background:linear-gradient(135deg,#002D63 0%,#003F8A 100%);color:#fff">
        <div class="card-body" style="padding:var(--s-3) var(--s-4);display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:var(--s-2)">
          <div style="display:flex;align-items:center;gap:var(--s-2)">
            ...
            <div>
              <strong>குரல் தட்டச்சு உதவி (Tamil & English Voice Assistant)</strong>
              <div style="font-size:11px;color:rgba(255,255,255,0.85)">எந்தவொரு கட்டத்திலும் கிளிக் செய்து பேசினால் தானாக தட்டச்சு செய்யப்படும் (Click field & speak)</div>
            </div>
          </div>
          <div style="display:flex;gap:var(--s-2);align-items:center">
            <button class="btn btn-sm btn-accent" id="members-mic-btn" onclick="VoiceKeyboard.toggleListening()">🎙️ Voice Type</button>
      ...
      <div class="card">
        <div class="table-wrapper">
          <table class="data-table">
            <thead>
              <tr>
                <th style="min-width:200px">உறுப்பினர் (Member)</th>
                <th>குழு (Group)</th>
                <th>இல்லம் (House)</th>
                <th>புள்ளிகள் (Points)</th>
                <th>தகுதி நிலை (Status)</th>
                <th class="col-hide-mobile">சேர்க்கை தேதி</th>
                <th style="text-align:right;min-width:180px">செயல்கள் (Actions)</th>
              </tr>
            </thead>
